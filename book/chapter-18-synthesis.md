...
5. **ExtEFT and UV limits**: a low-energy EFT limit exists (for SM contact)
   ...
   strip additivity and the weak-tail-generator boundary, after which explicit
   half-sided-inclusion and null-stress premises yield local stress-energy
...
additivity and a weak tail generator without any separate propagation or
endpoint-control assumption, because those controls are carried by the MaxEnt
branch itself. Turning that weak-tail data into a local null energy density
$T_{kk}$ still requires the downstream half-sided-inclusion and null-stress
