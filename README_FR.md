...
L'OPH est un programme de reconstruction. Son paquet d'axiomes actuel est algébrique-quantique : algèbres de patchs, états, probabilités de type trace/Born sur les surfaces d'enregistrement déclarées et entropie généralisée font partie de la base de travail. L'OPH ne prétend pas encore dériver la mécanique quantique à partir de primitives pré-quantiques. Il utilise cette base d'information algébrique pour reconstruire l'univers effectif observé : espace-temps, structure de jauge, particules, enregistrements et synchronisation des observateurs y apparaissent comme des conséquences de la cohérence de recouvrement sur un écran holographique fini.
...
La mécanique quantique est traitée comme une description algébrique effective portée par l'architecture OPH plus profonde, et non comme un formalisme déjà dérivé ici depuis un substrat classique ou pré-quantique. Une reconstruction plus forte des espaces de Hilbert, des `C*`-algèbres ou des algèbres de von Neumann, des probabilités de Born, de la trace et de l'entropie depuis de simples enregistrements opérationnels relèverait d'un programme OPH-QM séparé.
