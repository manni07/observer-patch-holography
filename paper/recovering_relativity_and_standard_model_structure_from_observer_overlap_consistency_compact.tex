...
Theorem~\ref{thm:geosubnet} removes the old ambiguity about the target algebra: the continuum Lorentz claim is asked only on the extracted prime geometric subnet, not on the full operational algebra. What remains open on the UV side is therefore still a two-step chain on \(\mathcal A_n^{\mathrm{geo}}\), and neither step is an independent cap-isotropy selector or a Euclidean-regularity premise.
