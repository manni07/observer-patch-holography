...
\end{enumerate}
The genuinely noncentral fixed-cutoff branch, where the parallel obstruction criterion is \(q_\Sigma=0\), is handled separately by Theorems~\ref{thm:hgdatum}--\ref{thm:hgtransport} and is not itself the ordinary compact-group reconstruction theorem.
\end{assumption}
...
\begin{proof}
Items (i)--(iii) are collected from Theorem~\ref{thm:confluence}, Corollary~\ref{cor:lorentz}, Theorem~\ref{thm:einstein}, and Corollary~\ref{cor:einstein}. The compact gauge reconstruction is Theorem~\ref{thm:compactgauge}, applied only after the transportable sector colimit, its \(3+1\)-dimensional symmetric braiding, and its faithful bosonic fiber functor are supplied by Assumption~\ref{ass:gaugeprem}; on the central branch the matching loop-coherence burden is already part of that supplied transportability rather than an extra theorem-side premise. The preceding MaxEnt/local-Gibbs/collar-mixing package is not used there to prove existence of that colimit or to decide whether it is trivial or nontrivial. The product gauge structure up to finite quotient is Theorem~\ref{thm:sm}; and the final identification of the exact Standard Model quotient is Theorem~\ref{thm:hypercharge} together with Corollaries~\ref{cor:nc}, \ref{cor:ng}, and Proposition~\ref{prop:z6}.
\end{proof}
