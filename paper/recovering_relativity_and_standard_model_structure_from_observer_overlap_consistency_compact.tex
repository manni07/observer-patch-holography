...
\]
after transport to each fixed local collar model. Only after that witness is supplied can local weak-\(*\) extraction and GNS gluing emit the realized scaling-limit cap pair. Beneath that carried-collar witness, the sharper raw datum is the fixed-local-collar Markov/faithfulness package.
...
What it does \emph{not} yet do is emit the absolute normalization as a theorem. The normalized same-label overlap-defect weight section is already closed beneath the weighted-cycle shape, and the remaining freedom is one positive absolute-scale bridge scalar
\[
B_\nu:=\lambda_\nu q_{\mathrm{mean}}^{p_\nu}/m_{\star,\mathrm{eV}}
\]
...
\]
The current attached stack factors through \(q_e=q_{\mathrm{mean}}qbar_e\), so a \(qbar_e\)-only bridge collapse does not follow from the present corpus. The older selected-point scalar built from the weighted-cycle state is already internal to the emitted \(qbar_e\), \(\psi_{\mathrm{wc}}\), and \(\psi_\ast\) data and therefore is not the missing bridge-external object. The newer direct correction audit sharpens the target-containing compare-only bridge window to \(B_\nu\in[6.69600,6.71040]\) with midpoint \(6.70320\), improving on the older three-route corridor, but no theorem-grade absolute spectrum exists until that bridge law is internalized.
Above the best emitted proxy, the missing bridge can now be tracked as a near-unity positive correction
\[
C_\nu:=\frac{B_\nu}{I_\nu^{1/2}\,\widehat{\mathrm{ratio}}^{\,1/2}\,\mathrm{sum\_defect}^{-1}},
\]
whose live compare-only value is \(C_\nu^\ast=0.9995473560\). A narrower shortlist-consensus window \([6.70484,6.71259]\) is also on disk, but it stays diagnostic-only because it does not contain the live compare-only target.
...
\]
but that family does not emit \(B_\nu\) by itself. The sharpest current local interface for the missing bridge remains the Majorana overlap-defect scalar route, yet the actual open object above the normalizer is still the positive non-homogeneous bridge scalar \(B_\nu\), not another hidden branch selector. The strongest closed-form residual clue remains compare-only and must stay that way until derived.

The hadron lane remains separate. It depends on nonperturbative runtime completion and stable-channel convergence rather than on a closed theorem-level mass surface, so the neutrino weighted-cycle improvement does not promote hadron observables.
