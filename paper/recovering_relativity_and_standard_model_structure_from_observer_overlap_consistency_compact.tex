...
\begin{remark}
The five axioms fix the OPH core, and in the present packaging they already internalize the quasi-local propagation and refinement-stable notion of sector persistence used by the scaling-limit and gauge branches. On the gravity side this means the null bridge no longer imports a separate propagation, endpoint-control, or half-sided-inclusion premise: c.11 closes the fixed-cutoff strip argument through exact-or-controlled strip additivity, the weak tail generator, and the derived half-sided modular pair that Borchers--Wiesbrock promotes to a positive null translation generator. The cap-modular geometric action and the \(2\pi\) KMS normalization are recovered in Theorem~\ref{thm:bw} on the OPH geometric branch; what remains genuinely theorem-external is the retained eight-item technical package: the obstruction clause \textbf{T7} when transportability is invoked, the scaling-limit scope clause \textbf{T8}, fixed-cap stationarity \textbf{T9}, and the bosonic/symmetric/fiber-functor part of \textbf{T10}--\textbf{T12}, together with \textbf{T1} and \textbf{T4} where the fixed-cutoff regulator presentation and the geometric cap-modular branch hypothesis are used. In particular, the Lorentz/null-modular/Einstein and compact-gauge statements below are still scaling-limit claims rather than fixed-cutoff matrix-algebra identities.
\end{remark}
