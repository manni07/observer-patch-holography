...
The labeled premises used in the dependency table are introduced here before the ledger appears later in the paper. \textbf{T2} is the controlled refinement/scaling-limit scope clause for the Lorentz/null-modular/Einstein branch. The fixed-cap generalized-entropy stationarity result is Theorem~\ref{thm:equilibrium}, not a labeled premise. The former path-independent transportability premise \textbf{T1} is replaced by Theorem~\ref{thm:transport-from-gluing} and Corollary~\ref{cor:no-t1-premise}: ordinary/central strict transport is equivalent to vanishing of the central loop-coherence class \([z]_\Sigma\), and genuinely noncentral strict transport is equivalent to \(q_\Sigma=0\). The former fixed-cutoff bosonic-category premise \textbf{T3} is replaced by Theorem~\ref{thm:fixed-cutoff-bosonic-sector-category}. The former refinement/fiber premise \textbf{T4} is replaced by Theorem~\ref{thm:refinement-functor-fiber-descent}; the historical gauge-reconstruction slot now records only the remaining nontrivial-realization burden corresponding to \textbf{T5}.
...
\end{itemize}
The fixed-cap generalized-entropy stationarity result is Theorem~\ref{thm:equilibrium}. The historical gauge-reconstruction slot in Assumption~\ref{ass:gaugeprem} records the remaining nontrivial-realization burden after Theorems~\ref{thm:transport-from-gluing}, \ref{thm:fixed-cutoff-bosonic-sector-category}, and \ref{thm:refinement-functor-fiber-descent}.
\end{assumption}

\begin{assumption}[Historical gauge-reconstruction premise slot]\label{ass:gaugeprem}
This slot is retained only to preserve cross-reference stability during the premise-elimination
audit. It no longer supplies the refinement functor or fiber-descent data.

The former path-independent transportability premise \(\textbf{T1}\) is replaced by
Theorem~\ref{thm:transport-from-gluing}. The former fixed-cutoff bosonic-category premise
\(\textbf{T3}\) is replaced by Theorem~\ref{thm:fixed-cutoff-bosonic-sector-category}. The
former refinement/fiber premise \(\textbf{T4}\) is replaced by
Theorem~\ref{thm:refinement-functor-fiber-descent}, which constructs the faithful monoidal
\(^*\)-refinement functors, their composition coherence, cofinal preservation of
zero-obstruction transportable sectors, and the finite-dimensional bosonic fiber descent from
the OPH cofinal refinement datum and the fixed-cutoff collar-sector categories.

The only gauge-side burden not discharged by this slot is realized nontriviality of the
low-energy branch. If the constructed refinement-stable category contains only the tensor unit,
then Doplicher--Roberts/Tannaka reconstruction gives the trivial compact group. Nontrivial
realization is handled separately by the MAR branch and the later Standard-Model quotient theorem
stack; it is not hidden here as a refinement premise. The genuinely noncentral fixed-cutoff branch
is controlled by Theorems~\ref{thm:hgdatum}--\ref{thm:hgtransport} together with
Theorem~\ref{thm:transport-from-gluing}: \(q_\Sigma=0\) strictifies to the ordinary compact-group
transportable case, while \(q_\Sigma\ne0\) remains a higher-gauge sector and is not itself the
ordinary compact-group reconstruction theorem. Fermionic signs and chirality are not part of this
bosonic internal-gauge premise; they belong to the later fermionic/super-Tannakian matter lift.
\end{assumption}
...
\end{enumerate}
Thus the former \(\textbf{T1}\) burden is eliminated: overlap gluing constructs the transport
...
\begin{corollary}[No separate T1 premise]\label{cor:no-t1-premise}
...
\begin{corollary}[No fixed-cutoff T3 premise]\label{cor:no-t3-premise}
The old fixed-cutoff premise that the collar-sector packages are bosonic symmetric
\(C^*\)-tensor categories is replaced by Theorem~\ref{thm:fixed-cutoff-bosonic-sector-category}.
The remaining gauge-side premise burden is only the refinement/fiber/nontriviality burden: the
existence and faithfulness of the refinement functors, compatible finite bosonic fibers, and the
realized nontrivial low-energy branch.
\end{corollary}
...
\(\mathsf{Sect}^{\mathrm{bos}}_r\) are now theorem-produced and the refinement/fiber ladder is
...
\begin{proof}
The hypotheses required by Doplicher--Roberts / Tannaka reconstruction are now supplied by
