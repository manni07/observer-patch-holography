...
carrier: recovery dynamics on fixed-cutoff collars. The remaining burden is not to invent a repair
rule, but to prove that the accepted recovery-derived local moves are repair-complete, preserve
support-local disjoint commutation and restriction-compatible union-collar gluing on the declared
branch, and satisfy the Petz support/CPTP clause where that branch is used. The touched-overlap
local-fit contract already yields Lyapunov \(\Phi\)-descent on accepted moves.
