...
\begin{enumerate}[leftmargin=2em]
\item A conditional Jacobson-type Einstein branch is recovered from observer-overlap modular geometry, an explicit fixed-cap generalized-entropy stationarity premise, and an explicit null-modular / null-stress scaling-limit package; in the synchronized c.10 bridge, quasi-local propagation and endpoint control are now supplied internally up to the weak-tail-generator stage, while half-sided inclusion and null-stress identification remain explicit downstream premises.
...
The fixed-cutoff bridge needed for the gravity chain stops one step earlier than the later stress-tensor identification. At fixed regulator scale one can transfer the same cut-center data used in the spatial collar theorem to regulated null strips, and on one inherited strip model one can obtain exact or controlled four-term strip additivity together with endpoint-Lipschitz control of the renormalized half-line family. What is not proved here is the downstream density upgrade of the weak tail generator, the half-sided modular inclusion step, the Borchers promotion to a genuine positive translation generator, or the relativistic identification of the resulting density with a continuum \(T_{kk}\). The present subsection therefore establishes only the following c.10-local statements:
\begin{enumerate}[leftmargin=2em]
...
\end{enumerate}
The labels \(\ref{lem:density}\), \(\ref{cor:n2}\), \(\ref{lem:translation}\), and \(\ref{thm:stress}\) are retained below only as explicit forward-reference boundaries for the downstream c.11--c.13 tasks.
...
\begin{corollary}[Geometric form of the downstream half-sided inclusion step]\label{cor:n2}
...
\begin{lemma}[Downstream Borchers promotion template]\label{lem:translation}
...
[K,P]=i2\pi P.
\]
This lemma is a downstream Borchers--Wiesbrock step and is not proved from the fixed-cutoff strip arguments above.
\end{lemma}
...
This theorem is not established at the present c.10 step; it records the exact additional c.11--c.13 burden beyond the fixed-cutoff bridge proved above.
\end{theorem}
...
The c.10-local bridge is now closed exactly at the weak-tail-generator stage. What has been proved here is the fixed-cutoff transfer of cut-center data to null strips, the exact-or-controlled four-term strip relation on one inherited strip model, and endpoint-Lipschitz control of the renormalized half-line family strong enough to define the weak tail generator \(q(a,\Omega)\). The density upgrade of \(q\), the half-sided modular inclusion step, the Borchers promotion to a genuine positive translation generator, and the continuum null-stress identification remain explicit downstream c.11--c.13 tasks rather than hidden proof sketches.
\end{remark}
