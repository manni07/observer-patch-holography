...
\]
is the closed partition function obtained after gluing the open-edge weights $p_R(t)\propto d_R e^{-tC_2(R)}$. Peter--Weyl identifies this sum with the compact-group heat kernel at the identity, \(Z_{\mathrm{edge}}(t)=K_t(1)\), and the Chapman--Kolmogorov law gives the corresponding collar-sewing rule. The large-$N_{\mathrm{edge}}$ step is therefore not a separate analogy layer; it is a criterion-level theorem that states exactly when the standard Gross--Taylor large-$N$ rewriting of that two-dimensional Yang--Mills surface counts as a controlled effective description of the edge branch.
...
Assume the exact bridge identity \(Z_{\mathrm{edge}}(t)=K_t(1)\) above and a large-$N_{\mathrm{edge}}$
...
\begin{proof}
The exact bridge identity puts the edge partition function on the compact-group heat-kernel /
...
The theorem does not derive the large-\(N_{\mathrm{edge}}\) regime itself from the compact-gauge
