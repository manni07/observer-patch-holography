...
\]
and keeps it outside live particle predictions while the source spectral measure payload,
same-scheme remainder, and interval certificate are work in progress. The
measured endpoint calibration uses
\[
...
\]
for numeric displays only. It is an external calibration insert and does not
promote the source-side theorem. The
...
\]
through this outer formula. That value is carried in the measured-calibration
surface for tables and plots, with theorem promotion disabled. The five-layer
OPH trunk emits the source-side candidate
\[
P=1.63097209569432901817967892561191884270169,
\qquad
\alpha_{\mathrm{cand}}^{-1}=136.994835164621649457949994585787193262029.
...
structured Thomson continuation before the source spectral measure payload,
same-scheme remainder, and interval certificate become theorem-grade. On the current runtime
surface the declared-convention packet means scheme lock, threshold map, beta provenance, and interval composition. The
...
\]
The reason is mechanical: the same branch depends on the candidate \(P\)
root, the source spectral measure payload, and the interval certificate. The selected-carrier
...
the \(P\)-closure root plus electroweak \(W/Z\) chain remain nonclosed pending the source spectral measure payload and interval certificate. Hadrons are closed out-of-scope for this pipeline because the
...
\midrule
\(P\)-closure root & candidate root, not certified & no & source spectral measure payload + interval certificate & \(P=1.63097209569432901817967892561191884270169\), \(\alpha^{-1}_{\mathrm{cand}}=136.994835164621649457949994585787193262029\) \\
Measured fine-structure endpoint & external calibration insert & no & source spectral measure payload + interval certificate & \(\alpha^{-1}(0)=137.035999177(21)\), \(P=1.630968209403959324879279847782648941\ldots\) \\
Structural massless carriers & closed structural zero & yes & none & \(m_\gamma=m_g=m_{\mathrm{grav}}=0\) \\
Electroweak \(W/Z\) & compare-only frozen adapter, not a prediction theorem & no & source spectral measure payload + interval certificate & \(80.377~\mathrm{GeV}\), \(91.18797809193725~\mathrm{GeV}\) \\
...
Using the source-locked anchor from the runtime candidate,
\[
a_0(P)=128.308268045165213892552005990181778935450\ldots,
\]
the measured endpoint calibration gives
\[
...
supplied explicitly as compare-only metadata or as the measured calibration insert, not as an input to the closure solve.
