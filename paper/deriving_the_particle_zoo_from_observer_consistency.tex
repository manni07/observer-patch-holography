...
normalization, while the neutrino continuation branch closes numerically but remains on a
quantitatively wrong physical branch. Hadrons remain execution-bound. The purpose of the paper is therefore not to claim full closure, but to give a concise
...
Charged leptons & continuation & ordered-log and support-extension derivation & no promoted masses & centered charged shape is closed, but the determinant-normalization transport scalar \(\Delta_e^{\mathrm{abs}}\) remains open; theorem-grade waiting set is \(\widehat C_e\) then the common-refinement transport equalizer \\
Neutrinos & continuation & intrinsic Majorana eta-chain and PMNS continuation closure & no promoted masses & numerical continuation branch closes, but branch repair is needed to reach physical PMNS angles and mass splittings \\
...
Quark family & up quark & overlap continuation & reported value \(0.00228766~\mathrm{GeV}\); stronger two-scalar candidate \(0.00215992~\mathrm{GeV}\) & derive the value laws for \(\Delta_{ud}^{\mathrm{overlap}}\) and \(\eta_Q^{\mathrm{centered}}\); CKM/CP transport already closes on the D12 branch \\
Quark family & down quark & overlap continuation & reported value \(0.00512144~\mathrm{GeV}\); stronger two-scalar candidate \(0.00470005~\mathrm{GeV}\) & same overlap-continuation value laws; CKM/CP transport already closes on the D12 branch \\
Quark family & strange quark & overlap continuation & reported value \(0.0916111~\mathrm{GeV}\); stronger two-scalar candidate \(0.0935045~\mathrm{GeV}\) & same overlap-continuation value laws; CKM/CP transport already closes on the D12 branch \\
Quark family & charm quark & overlap continuation & reported value \(1.256692~\mathrm{GeV}\); stronger two-scalar candidate \(1.272939~\mathrm{GeV}\) & same overlap-continuation value laws; CKM/CP transport already closes on the D12 branch \\
Quark family & bottom quark & overlap continuation & reported value \(3.820118~\mathrm{GeV}\); stronger two-scalar candidate \(4.182754~\mathrm{GeV}\) & same overlap-continuation value laws; CKM/CP transport already closes on the D12 branch \\
Charged leptons & electron & continuation gap & \(n/a\); continuation pair \((\eta,\sigma)=(-6.72959,\,8.15406)\) gives near-exact centered-log shape closure, but the absolute scale remains underdetermined; compare-only target \(g_e^\star=0.0457789\), equivalently \(\Delta_e^{\mathrm{abs},\star}=3.00398633\) & emit the charged sector-response functor \(\widehat C_e\), then the common-refinement transport equalizer, then the induced determinant-normalization scalar \(\Delta_e^{\mathrm{abs}}\) and absolute scale \(g_e\) \\
Charged leptons & muon & continuation gap & \(n/a\); same centered-shape closure and absolute-scale underdetermination as the electron row & same \(\widehat C_e \rightarrow\) equalizer \(\rightarrow \Delta_e^{\mathrm{abs}} \rightarrow g_e\) closure chain \\
Charged leptons & tau lepton & continuation gap & \(n/a\); same centered-shape closure and absolute-scale underdetermination as the electron row & same \(\widehat C_e \rightarrow\) equalizer \(\rightarrow \Delta_e^{\mathrm{abs}} \rightarrow g_e\) closure chain \\
Neutrinos & \(\nu_e\) & continuation branch-repair gap & \(n/a\); live continuation branch emits \(\theta_{12}=0.78680\), \(\theta_{23}=0.04431\), \(\theta_{13}=0.04366\), \(\Delta m_{21}^2=1.789\times10^{-25}\,\mathrm{GeV}^2\), \(\Delta m_{31}^2=1.014\times10^{-24}\,\mathrm{GeV}^2\), but these are far from the physical oscillation pattern & repair the flavor-side realized-arrow pullback and, if necessary, the intrinsic neutrino ansatz so the emitted branch reaches the physical PMNS/splitting regime \\
Neutrinos & \(\nu_\mu\) & continuation branch-repair gap & \(n/a\); same live continuation branch as the electron-neutrino row & same branch-repair frontier \\
Neutrinos & \(\nu_\tau\) & continuation branch-repair gap & \(n/a\); same live continuation branch as the electron-neutrino row & same branch-repair frontier \\
...
\]
On the current \(t_1/5\) branch this yields the invariant package
\[
...
\begin{enumerate}[leftmargin=2em]
\item continuation-level value laws for \(\Delta_{ud}^{\mathrm{overlap}}\) and \(\eta_Q^{\mathrm{centered}}\); and
\item the quantitative question of whether the current D12 continuation branch, despite being transport-closed, lands near the physical CKM matrix.
\end{enumerate}
...
\item the broader honest overlap-continuation frontier consists of the two mass-side value laws \(\Delta_{ud}^{\mathrm{overlap}}\) and \(\eta_Q^{\mathrm{centered}}\), while CKM/CP transport is already closed on the current D12 branch.
\end{enumerate}
...
residuals, but the theorem-grade waiting set is sharper than that surface suggests. Once
\(\widehat C_e\) exists, \(\eta_{\mathrm{ext}}\) and \(\sigma_{\mathrm{ext}}\) become charged
spectral invariants rather than separate primitive goals, and the absolute-scale burden is pushed
to the common-refinement transport equalizer.
...
The smallest honest theorem object beyond this shell is therefore not another ad hoc scalar fit for
\(g_e\), but the determinant-normalization transport scalar \(\Delta_e^{\mathrm{abs}}\) emitted by
the common-refinement transport equalizer, with
\[
g_e = g_{\mathrm{ch}}^{\mathrm{shared}}\,e^{-\Delta_e^{\mathrm{abs}}}.
\]
...
\(\eta_{\mathrm{ext}}\), \(\sigma_{\mathrm{ext}}\), \(\Delta_e^{\mathrm{abs}}\), and \(g_e\)
they would induce.
...
The flavor rows are still \(n/a\), but the open boundary is no longer a missing downstream
matrix product. On the current continuation branch, the same-label scalar certificate,
the intrinsic mass-eigenstate bundle, the shared charged-lepton left basis, and the PMNS matrix
all close numerically. The problem is instead that this emitted branch is quantitatively wrong:
it does not land near the observed PMNS angles or mass splittings, so the honest remaining burden
is branch repair rather than another missing readout shell.

The live continuation branch emits
\[
\theta_{12}=0.7868015993137272,\qquad
\theta_{23}=0.04431419146853922,\qquad
\theta_{13}=0.043660528425690105,
\]
\[
\delta_{\mathrm{PMNS}}=-2.6070435701394157,\qquad
J=-4.91099474128153\times 10^{-4},
\]
and intrinsic splittings
\[
\Delta m_{21}^2=1.788985827699037\times 10^{-25}\ \mathrm{GeV}^2,
\qquad
\Delta m_{31}^2=1.0143949544256282\times 10^{-24}\ \mathrm{GeV}^2.
\]
These outputs are exact on the present continuation branch, but they are far from the physical
oscillation pattern. The neutrino lane therefore remains unpromoted not because the downstream map
is incomplete, but because the emitted branch is the wrong one.
...
The honest remaining task is no longer ``emit PMNS from the shared basis.'' That downstream map is
already exact on the current continuation branch. The real remaining burden is to repair the branch
that feeds that map so the emitted neutrino observables move into the physical regime. Concretely,
the continuation branch still needs:
\begin{enumerate}
\item a repaired flavor-side realized-arrow pullback whose same-label overlap/gap data do not
collapse onto the present nonphysical anisotropy pattern;
\item a repaired same-label scalar certificate, or equivalent repaired centered eta-class, derived
from that improved flavor-side pullback; and
\item if the flavor-side repair is not enough by itself, the smallest intrinsic-theta or
Majorana-ansatz repair that breaks the present \(\theta_{12}\approx 45^\circ\) trap while keeping
the intrinsic map exact.
\end{enumerate}

So the open neutrino problem is a branch-selection and branch-repair problem. The current emitted
certificate and PMNS machinery are useful because they show exactly where the present continuation
branch fails; they are not yet promotable neutrino predictions.
