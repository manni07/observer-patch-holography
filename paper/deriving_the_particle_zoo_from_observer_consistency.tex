...
\begin{abstract}
Observer-Patch Holography asks how much of the particle spectrum can be organized by one shared local variable instead of a long fitted parameter list. Building on the OPH recovery of relativity and Standard Model structure, this paper derives the massless carriers, the \(W\) and \(Z\) masses, the Thomson-limit fine-structure constant, a precise Higgs mass, a precise running quark spectrum with explicit Yukawas, and a neutrino family in the observed hierarchy and mixing regime. Charged-lepton absolute masses remain open, and hadrons remain outside the closed package. The quantitative branch is built from one dimensionless pixel ratio \(P \equiv a_{\mathrm{cell}}/\ell_P^2\). A self-similar balance theorem identifies the golden ratio \(\varphi=(1+\sqrt5)/2\) as the exact symmetric equilibrium point of the total/bulk/edge hierarchy. A universe with durable records, structure, and dynamics cannot sit exactly at that point. The synthesis paper then fixes \(P\) by an outer/inner self-consistency condition: the outer pixel detuning away from \(\varphi\) is required to match the inner electromagnetic observation scale emitted by the same pixel. This particle paper takes that common \(P\) as the upstream quantitative input and tracks its downstream particle consequences.
\end{abstract}
