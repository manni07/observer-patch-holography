...
Observer-Patch Holography asks a concrete follow-up question after the gauge structure is fixed. Can one local screen constant organize the particle spectrum, or do the masses and mixings remain a collection of unrelated inputs?
...
The particle branch studied here is a one-parameter forward reconstruction problem. Instead of treating masses, mixings, and Yukawa parameters as independent inputs, it asks how far one common dimensionless pixel ratio \(P\) can be propagated through the whole spectrum. The working hypothesis is simple: fix one common \(P\) on the outer and inner closure branch of the synthesis paper, then use that same \(P\) through the bosonic, quark, charged-lepton, neutrino, and hadronic lanes. The structural massless carriers, the \(W\) boson and \(Z\) boson pair, the Higgs boson and top quark pair, the selected-class exact quark theorem, and the weighted-cycle neutrino theorem branch with its shared-basis physical Majorana pair are emitted, while charged leptons, flavor-labeled neutrinos, and hadrons remain work in progress. This paper develops that derivation chain from the axioms and \(P\) to the reported particle outputs, while keeping the open closure boundary explicit.
...
That continuation remains narrower than a derived unification theorem. The appendix records one
...
MSSM-like benchmark shift. It does not yet prove that those effective loop multiplicities,
...
\item the beta-shift package itself still sits on explicit D10 calibration assumptions rather than a closed edge-sector theorem;
