...
Cosmological constant problem & Phase II input-dependent global closure & local null data leave the Einstein equation fixed only up to a metric term, and the additional screen-capacity identification fixes that remaining branch constant by \(\Lambda=3\pi/(G N_{\mathrm{scr}})\). This is the global closure of the same Einstein branch, but it still lies outside the local recovered-core theorem because it requires the external input \(N_{\mathrm{scr}}\). \\
