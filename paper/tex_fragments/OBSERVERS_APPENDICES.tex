...
\bottomrule
\endfoot
Quantum gravity consistency & Phase I recovered core (conditional scaling-limit) & Conditional Lorentz branch on the BW scaling surface of the extracted prime geometric subnet, with the realized transported geometric cap-local system, its derived carried-collar schedule above the transported fixed-local-collar Markov/faithfulness datum, and the later ordered cut-pair rigidity step still explicit; the fixed-cutoff cap algebras are type-I regulators, but the scaling-limit observer algebra may be non-type-I/type III, so the theorem is automorphism-level and outer in the generic continuum case, with \(K_C=2\pi B_C\) only on the special type-I limit branch; conditional Jacobson-type Einstein branch from the null bridge, the separate bounded-interval projective branch, and fixed-cap stationarity. \\
