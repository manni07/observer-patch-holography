...
\textbf{Key conditionality.} The Lorentz/null-modular/Einstein package is a scaling-limit statement rather than a literal fixed-cutoff claim. Section 5 now removes any separate propagation or endpoint-control premise from the fixed-cutoff null bridge and closes that bridge only through the weak-tail-generator boundary; the downstream half-sided modular-inclusion and relativistic null-stress steps remain explicit scaling-limit burdens rather than hidden assumptions. The gauge group reconstruction yields \emph{a} compact group from a refinement-stable sector colimit; the explicit structural-economy selector MAR (Minimal Admissible Realization) then selects the SM gauge group \(SU(3) \times SU(2) \times U(1)/\mathbb{Z}_6\), \(N_c = 3\), and \(N_g = 3\) once the admissible class includes one connected abelian charge factor (see §6.2 and Part II of this manuscript).
...
\begin{itemize}
\item
\textbf{N1 (fixed-cutoff null-strip bridge).} The synchronized c.10 package now proves transferred cut-center data, theorem-local inherited left/right strip structure, exact-or-controlled four-term strip additivity on one inherited strip model, and renormalized endpoint control up to the weak tail generator. No separate LR or endpoint-control premise is imported beyond that theorem-local inherited-strip hypothesis.
\item
\textbf{N2 (half-sided modular inclusion).} Nested null half-line algebras satisfy half-sided modular inclusion; once this scaling-limit premise is granted, Borchers/Wiesbrock gives the translation group with positive generator; see \href{https://projecteuclid.org/journals/communications-in-mathematical-physics/volume-157/issue-1/Half-sided-modular-inclusions-of-von-Neumann-algebras/cmp/1104253848.pdf}{Wiesbrock, CMP 157 (1993)}.
\item
...
This subsection now stops at the fixed-cutoff weak-tail-generator boundary. At fixed cutoff one can transfer cut-center data to regulated null strips and obtain exact or controlled strip additivity on one inherited strip model. What is not proved here is the downstream density upgrade, the half-sided modular inclusion step, the Borchers promotion to a genuine positive translation generator, or the relativistic null-stress identification. Accordingly the c.10-local bridge consists only of:
\begin{itemize}
\tightlist
\item
...
\end{itemize}
The later density formula, Borchers generator, and \(T_{kk}\) bridge are kept below only as explicit downstream templates rather than as fixed-cutoff theorems proved in this subsection.
...
\textbf{Corollary 5.2e (Geometric form of the downstream N2 premise).} N2, the half-sided modular-inclusion premise on null half-lines, remains explicit. The only fixed-cutoff statement made here is that on the geometric modular branch the blow-up of Theorem 4.2 identifies its expected null-dilation form:
\[
v-v_0\mapsto e^{-2\pi t}(v-v_0).
\]
This does not derive N2 from strip data.

\textbf{Lemma 5.2f (Downstream Borchers promotion template).} Under N2, Borchers--Wiesbrock promotes the weak tail generator to a genuine one-parameter unitary group \(U(a)=e^{iaP}\) with \(P\ge0\) and
\[
\Delta^{it}U(a)\Delta^{-it}=U(e^{-2\pi t}a),
\qquad
[K,P]=i2\pi P.
\]
This is a downstream step and is not proved in the present subsection.

\textbf{Theorem 5.2g (Downstream null-stress identification template).} If one adds the density-upgrade hypotheses of Lemma 5.2d, the half-sided modular-inclusion premise N2, and the relativistic scaling-limit null-stress premise, then one expects an operator-valued distribution \(T_{kk}(v,\Omega)\) with
\[
\widetilde K[(a,\infty),\Omega]
=
2\pi \int_a^\infty (v-a)\,T_{kk}(v,\Omega)\,dv
+
R^{\mathrm{down}}_{(a,\infty),\Omega},
\]
...
+
R^{\mathrm{down}}_{I,\Omega}.
\]
Here the downstream remainder records the still-unproved density-upgrade error together with the fixed-strip Markov or recovery defects and the EFT-side approximation error carried from the fixed-cutoff bridge. The present subsection does not prove this theorem; it records the exact downstream burden beyond the fixed-cutoff bridge.
...
\item
  the local finite-constraint MaxEnt branch gives endpoint-Lipschitz control strong enough to define a weak tail generator for renormalized half-lines before any Borchers theorem is invoked;
\item
  the density upgrade, N2, Borchers promotion, and the final identification with a local continuum \(T_{kk}\) are all explicit downstream steps.
\end{itemize}
This is the disciplined route from null-strip factorization to the later Jacobson branch without pretending that c.10 has already completed c.11--c.13.
...
Once the fixed-cutoff null modular structure of Section 5.2 is combined with the downstream density, half-sided-inclusion, and null-stress premises, focusing constraints follow from information-theoretic principles in the same scaling-limit branch rather than from a separate fixed-cutoff postulate.
...
\item
  Downstream density upgrade + N2 (half-sided inclusion) ⟹ half-line modular densities and {[}K, P{]} = i 2π P
\item
...
\begin{quote}
\textbf{Proposition 5.10a (Conditional internal QNEC).} Under the fixed-cutoff null bridge of §5.2 together with the downstream density-upgrade, half-sided modular-inclusion, and resulting null-stress identification premises, the second null variation of von Neumann entropy satisfies
...
\textbf{Proof.} Assume the downstream half-sided modular-inclusion premise N2 in the form recorded in Corollary 5.2e and its Borchers-Wiesbrock consequence from Lemma 5.2f. This gives the translation structure with \([K, P] = i 2\pi P\).
