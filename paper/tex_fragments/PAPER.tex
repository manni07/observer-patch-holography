...
\item \textbf{String/worldsheet reorganization}: the exact heat-kernel bridge fixes the edge partition function on the two-dimensional Yang--Mills surface, and the compact paper gives a theorem-level criterion for when the standard Gross--Taylor large-\(N\) rewriting upgrades the large-\(N_{\mathrm{edge}}\) branch to a controlled effective description once one fixes a distinct large-\(N_{\mathrm{edge}}\) sequence, a compact fixed-\(\tau=tN_{\mathrm{edge}}\) window, and uniform genus-remainder control. Any further lift to critical superstring structure remains a conjectural extension requiring extra worldsheet ingredients.
