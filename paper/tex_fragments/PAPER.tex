...
the accepted recovery-derived moves are repair-complete for overlap consistency, preserve the
support-local disjoint-commutation and restriction-compatible union-collar glue that turn overlap
associativity into the local diamond on the physical quotient, and are controlled on the declared
Petz domain. The declared
touched-overlap acceptance contract is already what makes \(\Phi\) the finite-patch Lyapunov
functional for accepted moves.
