...
The fixed-cutoff null bridge proved at this stage stops at the weak tail generator. What is established here is: transferred cut-center data on null strips, exact-or-controlled strip additivity on one inherited strip model, and endpoint-Lipschitz control of the renormalized half-line family. What is not established here is the downstream density upgrade, the half-sided modular inclusion step, the Borchers promotion to a genuine positive translation generator, or the relativistic null-stress identification. The present condition package should therefore be read with the following split.
...
\textbf{Condition N2 (half-sided modular inclusion).} N2 remains a separate scaling-limit premise. The only fixed-cutoff statement made here is geometric: if a later task establishes half-sided modular inclusion on the null half-line blow-up algebras, then the geometric blow-up of cap modular flow identifies its expected form
\[
v-v_0\mapsto e^{-2\pi t}(v-v_0).
\]
This section does not derive N2 from strip data.
...
\]
This is the end of the present fixed-cutoff bridge. The stronger assumptions
\[
...
\textbf{Consequence.} The present condition package proves only the fixed-cutoff strip bridge through the weak tail generator \(q(a,\Omega)\). A genuine positive null translation generator requires the downstream Borchers--Wiesbrock step under N2, and identification with a continuum \(T_{kk}\) requires in addition the density-upgrade hypotheses and the relativistic EFT null-stress premise. Those later steps are explicit downstream burdens rather than hidden parts of N1--N3.
