...
The null-strip package now separates three logically distinct layers: transferred cut-center data, an additional decomposition-inheritance condition on strip multiplicity spaces, and exact or controlled exact-Markovity on one fixed inherited strip model. This is the precise null analogue of the spatial collar factorization theorem.

\textbf{Condition N1 (null-strip center transfer, inherited split, and exact-or-controlled additivity).} Fix a regulated null tripartition
\[
I_-=(v_1,v_2),\qquad
J=(v_2,v_3),\qquad
I_+=(v_3,v_4),
...
\right\}.
\]
Exact Markovity then gives
\[
K_{I_-\cup J\cup I_+}
=
K_{I_-\cup J}+K_{J\cup I_+}-K_J+K_{\partial,J},
\qquad
K_{\partial,J}\in Z(\mathcal A(J)).
\]
If strip CMI is only \(\le \varepsilon\), exact identities require a controlled exact-Markov replacement with carried error
\[
r_{\mathrm{FR}}(\varepsilon)
+
4\lambda_\ast^{-1}\delta^{\mathrm M}_{I_-:J:I_+}(\varepsilon),
\]
where \(r_{\mathrm{FR}}(\varepsilon)=2\sqrt{1-e^{-\varepsilon}}\le 2\sqrt{\varepsilon}\). Small strip CMI alone is therefore not silently upgraded to exact splice formulas.

\textbf{Condition N2 (half-sided modular inclusion remains a separate scaling-limit premise).} Null half-line algebras satisfy the Borchers--Wiesbrock half-sided inclusion property only as an explicit premise in the scaling regime. The blow-up of geometric cap modular flow near a smooth cut identifies its geometric form,
\[
v-v_0 \mapsto e^{-2\pi t}(v-v_0),
\]
but does not derive N2 from fixed-cutoff strip factorization.

\textbf{Condition N3 (endpoint control and half-line differentiability).} Bounded-interval modular Hamiltonians obey the endpoint-Lipschitz estimate
\[
\bigl|\langle\psi,(K[I']-K[I])\phi\rangle\bigr|
\le
C_{\psi,\phi}\,|I'\Delta I|,
\]
from the branch-internal quasi-local propagation of the local MaxEnt generator. In addition, the renormalized half-line family \(\widetilde K_a(\Omega)\) must satisfy the weak twice-differentiability and vanishing-at-infinity assumptions needed for
\[
...
Under N1--N3 and the relativistic EFT identification premise, the renormalized half-line modular Hamiltonian takes the form
\[
...
E^{(\eta)}_{I,\Omega},
\]
where \(g_I(v)\) is affine-covariant and vanishes at finite interval endpoints. Thus the supplement now makes explicit that the null bridge uses the same theorematic center decomposition as the spatial collar branch only after center transfer and decomposition inheritance are both imposed, while N2 and the final continuum \(T_{kk}\) identification remain separate scaling-limit premises.
