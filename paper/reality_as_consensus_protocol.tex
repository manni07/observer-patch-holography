...
\(\Phi\) on accepted moves, so what remains above that step is repair completeness, local
...
local-fit contract of Definition~\ref{def:localfit}. What the present theorem package still
assumes rather than derives is repair completeness and local confluence. On the Petz branch, full
...
contract already gives Lyapunov \(\Phi\)-descent on accepted moves. The point here is also that no
